...
if (is_post()) {
    ...
    } elseif (!is_unique('voucher', 'code', $code)) {
    ...
    if (!$_err) {
        $stm = $pdo->prepare("INSERT INTO voucher
            (code, discount_type, discount_value, max_discount, min_spend, max_uses, one_per_member, valid_from, valid_until, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stm->execute([
            $code,
            $discount_type,
        ...
        temp('info', "Voucher '$code' added successfully.");
...
<form method="post" novalidate>
    <table class="form-table">
        <tr>
            <td>Code</td>
            <td><?= html_text('code', "placeholder='e.g. WELCOME10' style='text-transform:uppercase'") ?> <?= err('code') ?></td>
        </tr>
        <tr>
            <td>Discount Type</td>
            <td><?= html_select('discount_type', ['Fixed' => 'Fixed Amount (RM)', 'Percentage' => 'Percentage (%)'], 'Choose a type') ?> <?= err('discount_type') ?></td>
        </tr>
        <tr>
